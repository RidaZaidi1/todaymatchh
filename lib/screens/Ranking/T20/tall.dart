...
class Tallround extends StatefulWidget {
  const Tallround({ Key? key }) : super(key: key);

  @override
  State<Tallround> createState() => _TallroundState();
}
final controller = Get.put(DarkModeController());
class _TallroundState extends State<Tallround> {
 void initState() {
    super.initState();
    getSeries();
  }
  ...
  getSeries() async {
    ...
    print(data["response"]["ranks"]['all-rounders']['t20s']);
    for (var i = 0;
        i < data["response"]["ranks"]['all-rounders']['t20s'].length;
        i++) {
      OdiLists.add(data["response"]["ranks"]['all-rounders']['t20s'][i]);
