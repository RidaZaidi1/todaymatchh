...
class Tteam extends StatefulWidget {
  const Tteam({ Key? key }) : super(key: key);

  @override
  State<Tteam> createState() => _TteamState();
}
final controller = Get.put(DarkModeController());
class _TteamState extends State<Tteam> {
  void initState() {
    super.initState();
    getSeries();
  }
  ...
  getSeries() async {
    ...
    print(data["response"]["ranks"]['teams']['t20s']);
    for (var i = 0;
        i < data["response"]["ranks"]['teams']['t20s'].length;
        i++) {
      OdiLists.add(data["response"]["ranks"]['teams']['t20s'][i]);
